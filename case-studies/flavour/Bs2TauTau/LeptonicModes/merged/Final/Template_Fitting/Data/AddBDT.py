...
def Add_BDT(rdf,BDTModel,varlist):
    ...
    rdf = rdf.Define("nVertex_sighemi_2pi","float(Vertex_sighemi_2pi_ind.size())")
    rdf = rdf.Define("nVertex_sighemi_ppi","float(Vertex_sighemi_ppi_ind.size())")
...
r.gInterpreter.ProcessLine(f'''
    ...
    computeModel1 = TMVA::Experimental::Compute<{len(ModelsDict[BDT_Name])}, float>(bdt);
''')
