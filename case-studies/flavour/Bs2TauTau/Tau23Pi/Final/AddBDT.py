...
TMVA::Experimental::RBDT<> bdt("Bs2TauTau_Stage2_BDT", "/afs/cern.ch/work/t/tomonnar/public/Bs2TauTau/Stage2/BDT/Models/xgb_bdt_Baseline_NoMass_NoBug.root");
...
def Add_BDT(rdf):
    ...
    rdf = rdf.Define("diTauPlus_FlightDistance", "sqrt(pow(diTauPlus_x-Bs2TauTau_x,2) + pow(diTauPlus_y-Bs2TauTau_y,2) + pow(diTauPlus_z-Bs2TauTau_z,2))")\
             .Define("diTauPlus_Lifetime",       "float(1.7769/diTauPlus_p*diTauPlus_FlightDistance/299792458e3)")\
             ...
             .Define("diTauMinus_Lifetime",      "float(1.7769/diTauMinus_p*diTauMinus_FlightDistance/299792458e3)")
    ...
    rdf = rdf.Define("Bs_FlightDistance","Get_Norm(Bs2TauTau_x-PV_x,Bs2TauTau_y-PV_y,Bs2TauTau_z-PV_z)")\
             ...
             .Define("Bs_Lifetime","float(5.3669/Bs_p*Bs_FlightDistance/299792458e3)")

    #Get the various mass variable, but not the collinear mass on
    rdf = rdf.Define("DeltaM","float(mDiTau_Vis-diTauPlus_mass-diTauMinus_mass)")
    
    #Needed because the BDT knows only float and for some reasons does not give an error but afterward get trouble with Count() and GetValue() to get the efficiencies
    rdf = rdf.Redefine("mDiTau_Vis","float(mDiTau_Vis)")

    BDTvars = ['diTau_Angles', 'diTauPlus_IP', 'diTauPlus_IPV', 'diTauMinus_IP', 'diTauMinus_IPV', 

               'diTauPlus_Lifetime', 'diTauMinus_Lifetime', 'Bs_Lifetime', 'Bs_IPV']

    rdf = rdf.Define("MVAVec2", r.computeModel1,BDTvars).Define("MVA2", "MVAVec2.at(0)").Filter("MVA2 > 0.8686868686868687")

    return rdf


def Load_Snap_RDF(mode,amount,varlist):
    ...
    rdf = Add_BDT(rdf)
    rdf.Snapshot("events",f"/afs/cern.ch/work/t/tomonnar/public/Bs2TauTau/Final/Datasets/Baseline_NoMass_NoBug_BDT/{mode}.root",varlist)

def Do_AddBDT(sigamount):

    VarList = ["diTau_Angles",
               "diTauPlus_IP","diTauPlus_IPV",
               "diTauMinus_IP","diTauMinus_IPV",
               
               "diTauMinus_rho1mass","diTauMinus_rho2mass","diTauPlus_rho1mass","diTauPlus_rho2mass",
               "diTauPlus_mass","diTauMinus_mass",
               "diTauPlus_FlightDistance","diTauMinus_FlightDistance",
               "diTauPlus_Lifetime","diTauMinus_Lifetime",
               
               "Bs_Lifetime","Bs_FlightDistance",
               "Bs_IPV",
               "mDiTau_Vis","DeltaM","mDiTau_collinear3D","MVA2"
              ]
    ...
    for mode in ["sig","bb","cc","ss","ud"]:
        ...
        Load_Snap_RDF(mode,sigamount,columns)
